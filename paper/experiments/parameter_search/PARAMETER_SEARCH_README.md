...
```bash
./paper/experiments/parameter_search/run_parameter_search.sh
```

Or using Python directly with uv:

```bash
uv run python paper/experiments/parameter_search/parameter_search.py
```
...
uv run python paper/experiments/parameter_search/visualize_parameter_search.py

# Prioritize SMAPE over pass@1 (α=0.7, β=0.3)
uv run python paper/experiments/parameter_search/visualize_parameter_search.py --alpha 0.7 --beta 0.3

# Prioritize pass@1 over SMAPE (α=0.3, β=0.7)
uv run python paper/experiments/parameter_search/visualize_parameter_search.py --alpha 0.3 --beta 0.7
```
...
Edit `paper/experiments/parameter_search/parameter_search_config.yaml` to adjust:

- **Model**: Which model to use (default: gpt-4o-mini)
- **Sample Size**: Number of PDFs to process per experiment (default: 5)
- **Evaluation Set**: VAL, TEST, or FULL dataset
...
To add a new parameter to search:

1. Add it to `parameter_search_config.yaml`:
   ```yaml
   parameters:
     your_new_parameter:
...
2. Update `parameter_search.py` to handle the new parameter:
   - Add parameter handling in `reset_default_configs()`
   - Add experiment execution in `main()`

### Changing Evaluation Metrics

To use different evaluation metrics, modify the `evaluate_extraction_results()` function in `parameter_search.py`.

## Troubleshooting

- **Out of Memory**: Reduce `sample_size` in the config file
- **API Rate Limits**: Add delays between experiments or reduce parallel processing
- **Missing Ground Truth**: Ensure ground truth CSV files exist in `paper/data/processed/ground_truth/`

## Notes

- The script saves intermediate results after each experiment to prevent data loss
- Failed experiments are logged but don't stop the entire search
- Results are evaluated based on extraction quality metrics, not just speed
- The default configuration uses a small sample size (5 PDFs) for quick testing - increase for production use
