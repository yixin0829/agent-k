...
def load_parameter_search_results(
    base_dir: str = "paper/data/parameter_search",
    ...
    for param_name, dir_name in param_dirs.items():
        ...
        if os.path.exists(results_file):
            ...
            print(f"Warning: No results found for {param_name} at {results_file}")
...
def plot_parameter_performance(
    ...
    output_dir: str = "paper/data/parameter_search/plots",
    alpha: float = 0.5,
    beta: float = 0.5,
):
    """
    ...
    for idx, (param_name, df) in enumerate(results.items()):
        ...
        ax2 = axes[idx, 1]
        if (
            "execution_time" in metrics_df.columns
            and metrics_df["execution_time"].notna().any()
        ):
            ax2.plot(
            ...
            ax2.set_title(f"{param_name}: Execution Time")
    ...
    # Save plot
    plot_file = os.path.join(output_dir, "parameter_search_results.png")
...
def create_summary_table(
    results: Dict[str, pd.DataFrame], alpha: float = 0.5, beta: float = 0.5
) -> pd.DataFrame:
    """
    ...
    for param_name, df in results.items():
        ...
        if (
            ...
            best_idx = metrics_df["execution_time"].idxmin()
            summary_row["Fastest"] = metrics_df.loc[best_idx, "parameter_value"]
            summary_row["Time (s)"] = (
                f"{metrics_df.loc[best_idx, 'execution_time']:.2f}"
            )
...
def main(alpha: float = 0.5, beta: float = 0.5):
    """
    ...
    # Load results
    print("Loading parameter search results...")
    results = load_parameter_search_results()

    if not results:
        print("No results found. Please run the parameter search first.")
        return
    ...
    if best_params:
        print("\n" + "=" * 80)
        print("BEST OVERALL PARAMETERS (Based on Weighted Composite Metric)")
        print(f"Composite Metric = {alpha} × (SMAPE) + {beta} × Pass@1")
        print("=" * 80)

        for param_name, info in best_params.items():
            print(f"\n- {param_name}:")
            print(f"   Best Value: {info['value']:.1f}")
            print(f"   Composite Score: {info['composite_score']:.4f}")
            print(f"   SMAPE: {info['smape']:.4f}")
            print(f"   Pass@1: {info['pass_at_1']:.4f}")
    ...
    if not summary_df.empty:
        print("\n" + "=" * 80)
        print("PARAMETER SEARCH SUMMARY (All Metrics)")
        print("=" * 80)
        print(summary_df.to_string(index=False))

        # Save summary table
        summary_file = "paper/data/parameter_search/parameter_summary_table.csv"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Weight for (SMAPE) in composite metric (default: 0.5)",
    )
    parser.add_argument(
        "--beta",
        type=float,
        default=0.5,
        help="Weight for Pass@1 in composite metric (default: 0.5)",
    )
    ...
    if args.alpha + args.beta != 1.0:
        ...
        print(f"Note: Weights normalized to α={args.alpha:.2f}, β={args.beta:.2f}")
