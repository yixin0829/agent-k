...
def load_data_and_process() -> pd.DataFrame:
    ...
    agent_extractions = [
        "paper/data/experiments/rag_batch_extraction/llama-3.3-70b/2025-08-09_20-33-26_batch_extraction.csv"
    ]
    ...
    ground_truth_path = "paper/data/processed/ground_truth/inferlink_ground_truth.csv"
...
def save_metrics(df_metrics: pd.DataFrame) -> None:
    ...
    """
    output_file = f"paper/data/eval/pdf_extraction_metrics_{get_curr_ts()}.csv"
...
def main() -> None:
    ...
    df_merged.to_csv(f"paper/data/eval/df_merged_{get_curr_ts()}.csv", index=False)
